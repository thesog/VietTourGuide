...
class Cp_PriceRangeController extends Zend_Controller_Action
{
    ...
    public function init ()
    {
        ...
        $this->model = new Cp_Model_PriceRange();
        $this->form = new Cp_Form_PriceRange();
    }
    ...
    public function listAction ()
    {
        ...
        $filter_keywords = isset($_SESSION['pricerange']['filter']['keywords']) ? $_SESSION['pricerange']['filter']['keywords'] : '';
        $filter_keywords = isset($_POST['filter_key']) ? $_POST['filter_key'] : $filter_keywords;
        $_SESSION['pricerange']['filter']['keywords'] = $filter_keywords;
        $filter['name'] = $_SESSION['pricerange']['filter']['keywords'];
        
        $order_field = isset($_SESSION['pricerange']['filter']['order_field']) ? $_SESSION['pricerange']['filter']['order_field'] : 'name';
        $order_dir = isset($_SESSION['pricerange']['filter']['order_dir']) ? $_SESSION['pricerange']['filter']['order_dir'] : 'DESC';
        if (isset($_POST['order_field']) && $_POST['order_field'] != '') {
            $order_field = $_POST['order_field'];
            $order_dir = $_POST['order_dir'];
        }
        $_SESSION['pricerange']['filter']['order_field'] = $order_field;
        $_SESSION['pricerange']['filter']['order_dir'] = $order_dir;
        
        $order['field'] = $_SESSION['pricerange']['filter']['order_field'];
        $order['dir'] = $_SESSION['pricerange']['filter']['order_dir'];
        ...
        if (isset($_POST['submit_option'])) {
            if ('ordering' == $_POST['submit_option']) {
                ...
                if ('delete' == $_POST['submit_option']) {
                    ...
                    return $this->_redirect('cp/price-range/list');
                } else 
    ...
    {
        $this->view->headTitle("Sửa khoản giá - KunaViet CMS");
    ...
    public function addAction ()
    {
        ...
        $this->view->headTitle("Thêm khoản giá - KunaViet CMS");
