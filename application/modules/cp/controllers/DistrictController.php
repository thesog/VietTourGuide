...
class Cp_DistrictController extends Zend_Controller_Action
{
    ...
    public function init ()
    {
        ...
        $this->model = new Cp_Model_District();
        $this->form = new Cp_Form_District();
    }
    ...
    {
        $this->view->headTitle("Quận huyện - KunaViet CMS");
        
        $order = array();
        $filter = array();
        
        $filter_keywords = isset($_SESSION['district']['filter']['keywords']) ? $_SESSION['district']['filter']['keywords'] : '';
        $filter_keywords = isset($_POST['filter_key']) ? $_POST['filter_key'] : $filter_keywords;
        $_SESSION['district']['filter']['keywords'] = $filter_keywords;
        $filter['name'] = $_SESSION['district']['filter']['keywords'];
        
        $order_field = isset($_SESSION['district']['filter']['order_field']) ? $_SESSION['district']['filter']['order_field'] : 'name';
        $order_dir = isset($_SESSION['district']['filter']['order_dir']) ? $_SESSION['district']['filter']['order_dir'] : 'DESC';
        if (isset($_POST['order_field']) && $_POST['order_field'] != '') {
            $order_field = $_POST['order_field'];
            $order_dir = $_POST['order_dir'];
        }
        $_SESSION['district']['filter']['order_field'] = $order_field;
        $_SESSION['district']['filter']['order_dir'] = $order_dir;
        
        $order['field'] = $_SESSION['district']['filter']['order_field'];
        $order['dir'] = $_SESSION['district']['filter']['order_dir'];
        ...
        if (isset($_POST['submit_option'])) {
            if ('ordering' == $_POST['submit_option']) {
                ...
                if ('delete' == $_POST['submit_option']) {
                    ...
                    return $this->_redirect('cp/district/list');
                } else 
    ...
    public function addAction ()
    {
        ...
        $this->view->headTitle("Thêm quận huyện - KunaViet CMS");
